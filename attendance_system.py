...
class AttendanceSystem:
    def delete_event(self, widget, event, data=None):             
        if self.is_admin or self.admin_ok(title=_("EXIT THE APPLICATION")):
            return False            
        else:
            return True
    ...
    def __init__(self, finger_print_device, database, db_cursor,\
        ...
        self.window.set_title(_("Assistance Control System "))
            ...
            if self.__user_section != None:            
                if not self.__user_section.unsave_datas():
                    ...
                    q_dialog = gtk.MessageDialog(self.window, gtk.DIALOG_MODAL, \
                                                ...
                                                " There are data without saving, if you leave the application data will be lost, do you want to continue?")
                    q_dialog.set_title(_("Assistance Control System "))
    ...
    def _show_error_dialog(self, message=""):
        ...
        e_dialog.set_title(_("Assistance Control System "))
        e_dialog.run()
        e_dialog.destroy()
    ...
    def __show_select_event_window(self):
        dialog = gtk.Dialog("Assistance Control System ", self.window,\
        ...
        #Add column to list
        events_treeview.append_column(gtk.TreeViewColumn(_("First name "), gtk.CellRendererText(), text=1))
    ...
    def create_main_toolbar(self):
        ...
        #Attendance
        self.tb_attendance = gtk.ToolButton(label=_(" Assistance "))
        ...
        self.tb_attendance.set_tooltip_text(_("Control Assistance (Ctrl+A)"))
        ...
        #Admin Options
        self.tb_config = gtk.ToolButton(label=_(" Administration "))
        self.tb_config.set_expand(True)
        self.tb_config.connect('clicked', self.admin_cb)
        self.tb_config.set_tooltip_text(_("Configure the Application (Ctrl+D)"))
        ...
        #Exit
        self.tb_exit = gtk.ToolButton(label=_(" Exit "))
        self.tb_exit.connect('clicked', self.exit_cb)
        self.tb_exit.set_expand(True)
        self.tb_exit.set_tooltip_text(_("Exit the application (Ctrl+E)"))
...
class SetupApplication():
    ...
    def start(self, splash_screen):
        if self.__config_param != None:
            splash_screen.update_progressbar(_("Starting Database..."))
            ok_db = self.__init_database(self.__config_param["database"])
            if ok_db:
                splash_screen.update_progressbar(_("Searching Fingerprint Device..."))
                self.__init_finger_print_device()
                if self.__config_param["app"]["attendance_by_finger_print"] == "True":                    
                    if self.__dev != None:
                        splash_screen.update_progressbar(_("Initiating Fingerprint Device..."))
                        self.__dev.open()
            else:
    ...
    def __open_finger_print_device(self):
        try:
            ...
            self.__setup_errors["mfp"] = _(" There was a problem when trying to start the fingerprint device, the application will not work correctly ")
    def __connect_db(self, db_param):
        try:
            ...
            self.__setup_errors["mdb"] = _(" There was a problem when trying to connect to the Database, the application will close")
            return False
